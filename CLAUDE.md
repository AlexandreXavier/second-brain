...
**Client.** Almost the entire UI lives in `src/App.tsx` (~30k). It subscribes to the shared `ideas` Firestore collection with `onSnapshot` and renders a masonry library; there is no router, no state library, and no component split beyond a few inline helpers. `src/lib/` holds the slim non-UI pieces:
...
**Functions (`functions/index.js`, CommonJS, Node 22).** Two HTTP entrypoints:
- `agentApi` — REST surface for agents over `/ideas` and `/ideas/:id` (GET list with optional `?search=`, POST, PATCH, DELETE) plus `/health`. Auth is the `x-agent-token` header: the function SHA-256-hashes it and looks up `agentTokens/{hash}` in Firestore. POST/PATCH go through `cleanIdeaPayload` which whitelists the same fields as `Idea`.
...
- `users/{uid}` — `UserProfile`, including the current `agentTokenHash` so the UI can show "you have an active token".
