...
Domain glossary for the Video Second Brain app — a shared visual library of video ideas used by a small team across web, desktop, and mobile.
...
**Platform**:
One of the three delivery surfaces: web, desktop, or mobile. All three share the same Firebase backend — same Firestore library, same Auth, same Storage bucket.
...
**Mobile app**:
The React Native client (bare CLI, `@react-native-firebase`). Ships Android first; iOS follows from the same codebase. Scope: browse the library and quick-capture (URL paste or camera roll). Does not include editing or deleting existing ideas.
_Avoid_: Android app, iOS app, phone app
...
**Quick capture**:
The mobile-specific capture flow: paste a URL to fetch a metadata preview, or pick an image from the camera roll. Writes directly to Firestore via `@react-native-firebase`, not through the agent API.
_Avoid_: Mobile capture, Android save

## Relationships

- A **Platform** reads and writes the shared **Library** via Firebase Auth + Firestore SDK (web, desktop, mobile) or via the `agentApi` Function (agents only)
- A **Capture** produces one **Idea** in the **Library**
- **Quick capture** is the **Capture** flow specific to the **Mobile app**
...
> **Dev:** "If I'm on the **mobile app** and I switch the **user scope filter** to 'Toda a equipa', will I see ideas saved by the **agent**?"
