...
async function getYouTubePreview(url: URL, originalUrl: string): Promise<IdeaDraft> {
  ...
  try {
    ...
    return {
      type: 'youtube', url: originalUrl,
      title: data.title || 'Video do YouTube',
      source: 'YouTube', sourceName: data.provider_name || 'YouTube',
    ...
    return {
      type: 'youtube', url: originalUrl,
      title: 'Video do YouTube', source: 'YouTube', sourceName: 'YouTube',
      thumbnailUrl: directThumbnail, imageUrl: directThumbnail,
      categories: [],
    };
  }
}
...
async function getTweetPreview(url: URL, originalUrl: string): Promise<IdeaDraft> {
  ...
  try {
    ...
    return {
      type: 'tweet', url: originalUrl,
      title: data.author_name ? `${data.author_name} no X` : 'Publicacao do X',
      source: 'X', sourceName: data.provider_name || 'X',
      author: handle || data.author_name,
      previewText: htmlToText(data.html) || originalUrl,
      categories: [],
    };
  } catch {
    return {
      ...
      source: 'X', sourceName: 'X', author: handle,
...
function getInstagramPreview(url: URL, originalUrl: string): IdeaDraft {
  const handle = parseSocialHandle(url);
  return {
    type: 'instagram', url: originalUrl,
    title: handle ? `Publicacao de Instagram de ${handle}` : 'Publicacao de Instagram',
    source: 'Instagram', sourceName: 'Instagram', author: handle,
...
async function getArticlePreview(url: URL, originalUrl: string): Promise<IdeaDraft> {
  ...
  try {
    ...
    return {
      type: 'article', url: originalUrl,
      title: meta?.title || source,
      source, sourceName: meta?.publisher || source,
    ...
    return {
      type: 'article', url: originalUrl,
      title: source, source, sourceName: source,
      description: originalUrl, categories: [],
    };
  }
}
...
export async function createPreviewFromUrl(value: string): Promise<IdeaDraft> {
  ...
  if (!url) {
    return { type: 'idea', title: value.trim() || 'Ideia solta', previewText: value.trim(), categories: [] };
  }
